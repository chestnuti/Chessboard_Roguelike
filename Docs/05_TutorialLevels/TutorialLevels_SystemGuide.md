...
- 地块类型、玩家出生点、敌人坐标、敌人阵营、敌人行为类型和击杀阈值全部由 DataAsset 固定配置。
- 运行时仍复用 `AGridManager`、`AGridPawn`、`AGridEnemyPawn`、`AGridEnemyManager`、`ATurnManager` 和现有战斗/能量规则。
- PCG 常规关卡路径保持默认不变。
...
| 资产 | 用途 |
| --- | --- |
| `Content/Data/Tutorial/DA_TutorialLevelSet.uasset` | 5 个教学关卡的固定布局和固定敌人配置 |
...
`UTutorialLevelSet` 构造函数内置了 5 个默认教学关卡。编辑器中的 `DA_TutorialLevelSet` 会基于这些默认数据创建，并额外绑定具体敌人蓝图类。
...
7. 初始化 `EnemyManager` 并将回合状态设置为 `PlayerInput`。
...
| `Tools/ValidateTutorialDataAsset.py` | 验证 DataAsset 中有 5 个 10x10 关卡、敌人数量、敌人类配置和第 5 关压制布局 |
