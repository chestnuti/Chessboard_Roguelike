...
| 拾取道具 | `AGridPickupActor::TryCollect` 实际消耗道具时 | `PlayerAudio.PickupItem` |
...
7. 若需要拾取道具反馈，配置 `PlayerAudio.PickupItem`。
8. 打开 `/Game/Data/Audio/Enemy` 下的敌人 Profile。
9. 为每个敌人事件槽位的 `Sounds` 数组添加对应音效。
10. 确认主菜单地图引用 `BP_GameAudioBootstrap_MainMenu`。
11. 确认关卡地图引用 `BP_GameAudioBootstrap_Level`。
12. 确认敌人蓝图上的 `AudioProfile` 指向对应敌人 Profile。
